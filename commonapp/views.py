...
def call_custom_function(func_name, *args, **kwargs):
    ...
    module_name = f"customapp_{idcliente}.custom_handlers"

    try:
        custom_handlers = importlib.import_module(module_name)
        if hasattr(custom_handlers, func_name):
            func = getattr(custom_handlers, func_name)
    ...
    for field_name, field_data in all_field_settings.items():
        ...
        if is_required or not is_editable:
            ...
            if is_required:
                ...
                if (norm_new_value is None or str(norm_new_value).strip() == '') and file_value is None:
...
def build_chart_data(request, chart_id, viewid=None, filters=None, block_category=None, viewMode=None, referenceYear=None, filtersList=None, searchTerm=""):
    ...
    if cliente_id == "wegolf":
        ...
        if block_category != "benchmark":
            ...
            dynamic_conditions.append(f"user_{from_table}.recordidgolfclub_ IN ('{club_list}')")
...
def _handle_aggregate_chart(request, config, chart_id, chart_record, query_conditions, viewMode=None, referenceYear=None, dashboard_category=None, from_sql_string=""):
    ...
    allowed_layouts = ['barchart', 'orizbarchart', 'stackedchart', 'multibarchart', 'overlappingbarchart', 'overlappedbarchart', 'multioverlappedbarchart']
    ...
    if str(active_server).lower() == 'wegolf':
        ...
        clubs = set()
        for r in dictrows:
            cid = r.get('recordidgolfclub_unique_')
            if cid:
                clubs.add(cid)
    ...
    # --- LOGICA CALCOLO SCALA OVERLAPPED BAR CHART GENERALIZZATA ---
    if chart_record.get('layout') in ['overlappedbarchart', 'multioverlappedbarchart'] and final_datasets1 and len(final_datasets1) >= 2:
        try:
            backgrounds = []
            foregrounds = []
            for idx in range(0, len(final_datasets1) - 1, 2):
                backgrounds.append(final_datasets1[idx].get('data', []))
                foregrounds.append(final_datasets1[idx+1].get('data', []))
            
            all_bg_values = []
            for bg in backgrounds:
                all_bg_values.extend([float(x) for x in bg if x is not None])
            max_bg = max(all_bg_values) if all_bg_values else 0
            
            raw_left_domain = max_bg * 1.1 if max_bg > 0 else 1
            max_left_domain = math.ceil(raw_left_domain / 100.0) * 100
            if max_left_domain == 0:
                max_left_domain = 100
                
            ratios = []
            for bg, fg in zip(backgrounds, foregrounds):
                if bg and fg and len(bg) == len(fg):
                    for i in range(len(bg)):
                        bg_val = float(bg[i]) if bg[i] is not None else 0
                        fg_val = float(fg[i]) if fg[i] is not None else 0
                        if bg_val != 0:
                            ratios.append((fg_val * max_left_domain) / bg_val)
                            
            all_fg_values = []
            for fg in foregrounds:
                all_fg_values.extend([float(x) for x in fg if x is not None])
            max_fg_real = max(all_fg_values) if all_fg_values else 0
            
            raw_right_new = (max(ratios) * 1.2) if ratios else (max_fg_real * 1.2)
            max_right_domain = math.ceil(raw_right_new / 100.0) * 100
            if max_right_domain == 0:
                max_right_domain = 100
                
            context['yAxisDomains'] = {
                'left': [0, int(max_left_domain)],
                'right': [0, int(max_right_domain)]
            }
        except Exception as e:
...
def get_form_fields(request):
    try:
        ...
        for field in sys_fields:
            ...
            group_translation = WegolfHelper.get_cached_translation(translations, field.get("label"), userid, translation_type="Label")
            if not group_translation:
                group_translation = field.get("label")
            group_name = group_translation

            # sub_group_name = field.get("sublabel")
            sub_group_translation = WegolfHelper.get_cached_translation(translations, field.get("sublabel"), userid, translation_type="Sublabel")
